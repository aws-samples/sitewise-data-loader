...
import { Partition, Partitions, PropertyValueStorageConfig } from './property-value-storage.models';
...
export class PropertyValueStorageService {
  ...
  async createPartition(tqvMessage: TqvMessage) {
    const partitionKey = this.hashHelper.getPartitionKey(tqvMessage.propertyAlias);
  ...
  async send(tqvMessages: TqvMessage[]) {
    ...
    for (let i = 0; i < tqvMessages.length; i++) {
      ...
      if (tempStorage[tqvMessage.propertyAlias] !== undefined) {
        if (tempStorage[tqvMessage.propertyAlias].propertyValues.length < 10) {
          tempStorage[tqvMessage.propertyAlias].propertyValues.push(tqvMessage.propertyValue);
        } else if (tempStorage[tqvMessage.propertyAlias].propertyValues.length === 10) {
