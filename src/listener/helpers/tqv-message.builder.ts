...
export class TqvMessageBuilder {
    ...
    public static buildTqvMessage(
        createMessageDto: CreateMessageDto
    ): TqvMessage {
        let tqvMessage: TqvMessage = {
    ...
    private static getTimeInNanos(
        createMessageDto: CreateMessageDto
    ): TimeInNanos {
        const current = Math.floor(Date.now() / 1000);
        const timeGap = current - createMessageDto.timeInSeconds;

        if (timeGap > TqvMessageBuilder.MAX_PAST_IN_EPOCH || timeGap < TqvMessageBuilder.MAX_FUTURE_IN_EPOCH) {
    ...
    private static getVariantValue(
        createMessageDto: CreateMessageDto
    ): Variant {
